...
void opt3001_get_data(I2C_Handle *i2c, double *bright) {
	...
	if (e & OPT3001_DATA_READY) {
		...
		if (I2C_transfer(*i2c, &i2cTransaction)) {
            ...
            Raw_OPT_Data = (rxBuffer[0] << 8) | (rxBuffer[1]);
		    // ACCORDING TO DATASHEET: GET BRIGHTNESS
    	    *bright = 0.0;
    	    int exp = Raw_OPT_Data >> 12;
    	    int factor = Raw_OPT_Data & 0x0FFFF;
    	    *bright = 0.01 * pow(exp, 2) * factor;
    	    }
            else {
